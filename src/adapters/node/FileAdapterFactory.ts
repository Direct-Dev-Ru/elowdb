...
export type FileType = 'json' | 'bson' | 'yaml';
...
export class FileAdapterFactory {
    static create<T>(
        ...
        switch (type) {
            case 'json':
                return sync === 'sync'
                    ? new JSONFileSync<T>(filename, _cypherKey, options as SyncFileAdapterOptions)
                    : new JSONFile<T>(filename, _cypherKey, options as AsyncFileAdapterOptions);
            case 'bson':
                return sync === 'sync'
                    ? new BSONFileSync<T>(filename, _cypherKey, options as SyncFileAdapterOptions)
                    : new BSONFile<T>(filename, _cypherKey, options as AsyncFileAdapterOptions);
            case 'yaml':
                return sync === 'sync'
                    ? new YAMLFileSync<T>(filename, _cypherKey, options as SyncFileAdapterOptions)
                    : new YAMLFile<T>(filename, _cypherKey, options as AsyncFileAdapterOptions);
            default:
