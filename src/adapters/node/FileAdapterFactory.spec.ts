...
describe('FileAdapterFactory', () => {
    ...
    describe('create', () => {
        it.each(['json', 'bson', 'yaml'] as FileType[])('should create %s adapter', (type) => {
            const adapter = FileAdapterFactory.create<TestData>(tempFile, type);
            expect(adapter).toBeDefined();
        });

        it.each(['sync', 'async'] as SyncType[])('should create %s adapter', (sync) => {
            const adapter = FileAdapterFactory.create<TestData>(tempFile, 'json', sync);
            expect(adapter).toBeDefined();
        });

        it('should throw error for unsupported file type', () => {
            expect(() => {
                FileAdapterFactory.create<TestData>(tempFile, 'unsupported' as FileType);
    ...
    describe('createJSON', () => {
        ...
        it('should create JSON sync adapter', () => {
            const adapter = FileAdapterFactory.createJSON<TestData>(tempFile, 'sync');
            expect(adapter).toBeDefined();
    ...
    describe('createBSON', () => {
        ...
        it('should create BSON sync adapter', () => {
            const adapter = FileAdapterFactory.createBSON<TestData>(tempFile, 'sync');
            expect(adapter).toBeDefined();
            expect(adapter).toBeInstanceOf(DataFileSync);
        });
    });
    ...
    describe('createYAML', () => {
        ...
        it('should create YAML sync adapter', () => {
            const adapter = FileAdapterFactory.createYAML<TestData>(tempFile, 'sync');
            expect(adapter).toBeDefined();
            expect(adapter).toBeInstanceOf(DataFileSync);
        });
    });
    ...
    describe('encryption', () => {
        it('should create encrypted adapter', async () => {
            const adapter = FileAdapterFactory.create<TestData>(tempFile, 'json', 'async', secretKey);
        ...
        it('should create encrypted sync adapter', () => {
            const adapter = FileAdapterFactory.create<TestData>(tempFile, 'json', 'sync', secretKey);
            const data: TestData = { id: 1, name: 'test' };
            adapter.write(data);
    ...
    describe('custom options', () => {
        it('should create adapter with custom encryption', async () => {
            const customEncrypt = async () => ({ error: 'Custom encryption failed' });
            const adapter = FileAdapterFactory.create<TestData>(tempFile, 'json', 'async', secretKey, {
                encrypt: customEncrypt
            });
            const data: TestData = { id: 1, name: 'test' };
            await expect(adapter.write(data)).rejects.toThrow('Custom encryption failed');
        });

        it('should create adapter with custom decryption', async () => {
            const customDecrypt = async () => ({ error: 'Custom decryption failed' });
            const adapter = FileAdapterFactory.create<TestData>(tempFile, 'json', 'async', secretKey, {
                decrypt: customDecrypt
            });
            
            await expect(adapter.read()).rejects.toThrow('Custom decryption failed');
        });
    });
}); 